...
try{
 ...
 await page.route('**/api/reports/schedules/**',route=>{
  ...
  if(action==='list')data={company:{id:3,name:'Sociedad de pruebas'},reports:[{code:'AR',name:'Cuentas por Cobrar',category:'Operativos'},{code:'AP',name:'Cuentas por Pagar',category:'Operativos'},{code:'general-ledger',name:'Libro Mayor',category:'Operativos'},{code:'bank-reconciliation',name:'Bancos',category:'Bancarios',requires_bank:true}],books:[{id:7,name:'Principal'}],banks:[{id:9,name:'Cuenta de prueba'}],schedules,history:[],transport:{configured:false,enabled:false}};
  if(action==='save'){const s={id:1,revision:1,subsidiary_id:3,name:p.name,report_kind:p.report,period_mode:p.periodMode,report_options:p.reportOptions,frequency:p.frequency,weekday:p.weekday,month_day:p.monthDay,send_time:p.time+':00',cutoff_rule:p.cutoff,recipients:p.recipients,format:p.format,active:p.active,next_run_at:'2026-10-31T15:30:00Z'};schedules=[s];data=s;}
 ...
 await f.locator('#new').click();await f.locator('#report').selectOption('general-ledger');assert.equal(await f.locator('#periodModeLabel').isVisible(),true);await f.locator('#periodMode').selectOption('LAST_7_DAYS');await f.locator('#book').selectOption('7');await f.locator('#preview').click();await f.locator('#message').filter({hasText:'Muestra descargada'}).waitFor();assert.equal(calls.filter(c=>c.action==='preview').at(-1).p.reportOptions.bookId,'7');assert.equal(calls.filter(c=>c.action==='preview').at(-1).p.periodMode,'LAST_7_DAYS');
 await f.locator('#report').selectOption('bank-reconciliation');assert.equal(await f.locator('#bankLabel').isVisible(),true);assert.equal(await f.locator('#bookLabel').isVisible(),false);await f.locator('#preview').click();await f.locator('#editor .dialogError').filter({hasText:'cuenta bancaria'}).waitFor();await f.locator('#bank').selectOption('9');await f.locator('#name').fill('Bancos semanal');await f.locator('#recipients').fill('finance@example.invalid');await f.locator('#save').click();await f.locator('#editor').waitFor({state:'hidden'});await f.locator('[data-edit]').click();assert.equal(await f.locator('#bank').inputValue(),'9');assert.equal(await f.locator('#periodMode').inputValue(),'LAST_7_DAYS');await f.locator('#cancel').click();
