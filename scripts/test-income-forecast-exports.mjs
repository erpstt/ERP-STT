...
assert.ok(csv.startsWith('\uFEFF'));assert.ok(!csv.includes('RUN_RATE'));assert.ok(!csv.includes('1:7:2026'));assert.ok(csv.includes('Resultado neto'));assert.ok(csv.includes('Contrato adicional'));
// Quoted fields may contain commas or line breaks; inspect the CSV as records.
const records=[];let record=[],cell='',quoted=false;for(let i=1;i<csv.length;i++){const ch=csv[i];if(ch==='"'){if(quoted&&csv[i+1]==='"'){cell+='"';i++;}else quoted=!quoted;}else if(ch===','&&!quoted){record.push(cell);cell='';}else if(ch==='\r'&&!quoted&&csv[i+1]==='\n'){record.push(cell);records.push(record);record=[];cell='';i++;}else cell+=ch;}record.push(cell);records.push(record);
