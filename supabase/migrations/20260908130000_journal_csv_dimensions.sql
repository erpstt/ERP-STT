-- Names are resolved only within the active subsidiary. ID:123 disambiguates names.
create or replace function public.journal_csv_lookup(p_kind text,p_value text) returns jsonb
language plpgsql security definer set search_path=public,pg_temp as $$
declare tab text; pk text; rel text; scope_sql text; result jsonb; n integer; sid bigint:=active_subsidiary_id();
begin
  if nullif(btrim(p_value),'') is null then return null; end if;
  case p_kind
    when 'Cliente' then tab:='customers'; pk:='customer_id'; rel:='entity_subsidiaries';
    when 'Proveedor' then tab:='suppliers'; pk:='supplier_id'; rel:='entity_subsidiaries';
    when 'Empleado' then tab:='employees'; pk:='employee_id';
    when 'departamento' then tab:='departments'; pk:='department_id'; rel:='department_subsidiaries';
    when 'centro_costos' then tab:='cost_centers'; pk:='cost_center_id';
    when 'clase' then tab:='classes'; pk:='class_id'; rel:='class_subsidiaries';
    when 'acreedor_financiero' then tab:='financial_creditors'; pk:='financial_creditor_id'; rel:='financial_creditor_subsidiaries';
    when 'compania_relacionada' then tab:='related_companies'; pk:='related_company_id'; rel:='related_company_subsidiaries';
    else raise exception 'Tipo de referencia no permitido: %.',p_kind;
  end case;
  if rel is null then scope_sql:='t.subsidiary_id=$2';
  else scope_sql:=format('exists(select 1 from %I s where s.%I=t.%I and s.subsidiary_id=$2)',rel,pk,pk); end if;
  execute format($q$
    select count(*),(jsonb_agg(j))->0 from (
      select to_jsonb(t) j from %I t where %s
    ) q where coalesce((j->>'is_inactive')::boolean,false)=false
      and coalesce((j->>'is_active')::boolean,true)=true
      and case when $1 ~ '^ID:[0-9]+$' then j->>%L=substring($1 from 4)
        else lower(coalesce(j->>'company_name',nullif(concat_ws(' ',j->>'first_name',j->>'last_name'),''),j->>'name'))=lower($1) end
  $q$,tab,scope_sql,pk) into n,result using btrim(p_value),sid;
  if n<>1 then raise exception '%: "%" no existe, está inactivo, no pertenece a la subsidiaria o es ambiguo. Use el nombre exacto o ID:identificador.',p_kind,p_value; end if;
  return result;
end$$;
revoke all on function public.journal_csv_lookup(text,text) from public,anon,authenticated;

create or replace function public.journal_csv_dimensions(p_line jsonb) returns jsonb
language plpgsql security definer set search_path=public,pg_temp as $$
declare kind text:=nullif(btrim(p_line->>'entidad'),''); entity jsonb; dept jsonb; center jsonb; cls jsonb; creditor jsonb; related jsonb; owner jsonb; eid bigint; did bigint;
begin
  if kind='Ninguna' then kind:=null; end if;
  if kind is not null and kind not in ('Cliente','Proveedor','Empleado') then raise exception 'Entidad debe ser Cliente, Proveedor, Empleado o Ninguna.'; end if;
  if kind is null and nullif(btrim(p_line->>'nombre'),'') is not null then raise exception 'Indique la entidad para completar nombre.'; end if;
  if kind is not null then
    if nullif(btrim(p_line->>'nombre'),'') is null then raise exception 'Complete nombre para la entidad %.',kind; end if;
    entity:=journal_csv_lookup(kind,p_line->>'nombre');
    eid:=coalesce(entity->>'customer_id',entity->>'supplier_id',entity->>'employee_id')::bigint;
  end if;
  dept:=journal_csv_lookup('departamento',p_line->>'departamento');
  if kind='Cliente' then
    did:=nullif(entity->>'department_id','')::bigint;
    if dept is not null and (dept->>'department_id')::bigint is distinct from did then raise exception 'El departamento no corresponde al cliente.'; end if;
    if did is not null then dept:=journal_csv_lookup('departamento','ID:'||did); end if;
  end if;
  if kind='Empleado' and dept is not null and dept->>'type' is distinct from 'Interno' then raise exception 'Para empleados seleccione un departamento Interno.'; end if;
  center:=journal_csv_lookup('centro_costos',p_line->>'centro_costos');
  cls:=journal_csv_lookup('clase',p_line->>'clase');
  if center is not null then
    owner:=journal_csv_lookup('Cliente','ID:'||(center->>'customer_id'));
    if dept is null or owner is null or owner->>'department_id' is distinct from dept->>'department_id' then raise exception 'El centro de costos no corresponde al departamento.'; end if;
    if kind='Cliente' and center->>'customer_id' is distinct from entity->>'customer_id' then raise exception 'El centro de costos no corresponde al cliente.'; end if;
    if cls is not null and cls->>'class_id' is distinct from center->>'class_id' then raise exception 'La clase no corresponde al centro de costos.'; end if;
    if nullif(center->>'class_id','') is not null then cls:=journal_csv_lookup('clase','ID:'||(center->>'class_id')); end if;
  elsif cls is not null then raise exception 'Indique un centro de costos para asignar la clase.';
  end if;
  creditor:=journal_csv_lookup('acreedor_financiero',p_line->>'acreedor_financiero');
  related:=journal_csv_lookup('compania_relacionada',p_line->>'compania_relacionada');
  return jsonb_build_object('entity_type',kind,'entity_id',eid,'department_id',dept->>'department_id','cost_center_id',center->>'cost_center_id','class_id',cls->>'class_id','financial_creditor_id',creditor->>'financial_creditor_id','related_company_id',related->>'related_company_id');
end$$;
revoke all on function public.journal_csv_dimensions(jsonb) from public,anon,authenticated;
  ...
  loop
    ...
    begin
      ...
      for item,row_number in select r,n::integer+1 from jsonb_array_elements(p_rows) with ordinality t(r,n) where r->>'asiento_referencia'=grp.reference order by n loop
        ...
        lines:=lines||jsonb_build_array(jsonb_build_object('account_id',aid,'debit',debit,'credit',credit,'note',item->>'nota_linea','service_month',nullif(item->>'mes_servicio','')) || journal_csv_dimensions(item));
      end loop;
