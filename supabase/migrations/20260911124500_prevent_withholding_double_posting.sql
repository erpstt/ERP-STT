...
begin
 ...
 if exists(select 1 from journal_line l join tax_codes tax_code on tax_code.tax_code_id=l.tax_code_id join tax_types tax_type on tax_type.tax_type_id=tax_code.tax_type_id where l.journal_id=j.journal_id and coalesce(l.tax_rate,tax_code.rate_percentage,0)>0 and coalesce(l.note,'') not like 'Retención %' and case when l.debit>0 then tax_type.asset_account_id else tax_type.liability_account_id end is null) then raise exception 'El tipo de impuesto seleccionado no tiene configurada la cuenta contable correspondiente.';end if;
 ...
 where l.journal_id=j.journal_id and coalesce(l.tax_rate,tax_code.rate_percentage,0)>0 and coalesce(l.note,'') not like 'Retención %' and case when l.debit>0 then tax_type.asset_account_id else tax_type.liability_account_id end is not null;
end$$;

do $$declare current_journal bigint;begin
 for current_journal in select distinct journal_id from supplier_invoice where journal_id is not null union select distinct journal_id from supplier_payment where journal_id is not null
 loop perform sync_journal_gl_impacts(current_journal);end loop;
end$$;
