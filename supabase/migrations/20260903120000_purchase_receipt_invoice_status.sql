update supplier_invoice si set purchase_receipt_document_id=d.document_id
from purchase_document d
where si.purchase_receipt_document_id is null and d.document_type='RECEIPT' and d.subsidiary_id=si.subsidiary_id and d.supplier_id=si.supplier_id and si.memo like '%'||d.document_number||'%'
and not exists(select 1 from supplier_invoice other where other.purchase_receipt_document_id=d.document_id);
