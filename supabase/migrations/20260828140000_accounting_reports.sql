...
select jsonb_build_object(
 'subsidiaries',coalesce((select jsonb_agg(jsonb_build_object('id',s.subsidiary_id,'name',s.name,'currencyId',s.currency_id) order by s.name) from subsidiaries s join allowed a using(subsidiary_id) where s.is_active),'[]'::jsonb),
 ...
 'currencies',coalesce((select jsonb_agg(jsonb_build_object('id',c.currency_id,'code',c.currency_code,'name',c.name) order by c.currency_code) from currencies c),'[]'::jsonb),
...
declare
 ...
 filter_department_id bigint; filter_location_id bigint; filter_class_id bigint; filter_cost_center_id bigint; filter_account_id bigint;
begin
 ...
 select count(*) into requested_count from unnest(sids) x;
 select count(*) into allowed_count from unnest(sids) x join user_subsidiaries u on u.subsidiary_id=x and u.user_id=app_user_id();
 if requested_count<>allowed_count or sids[1] is null then raise exception 'No tiene acceso a una o más subsidiarias seleccionadas.'; end if;
 ...
 if p_report in ('general-ledger','journal') then
  with detail as (
  ...
  with balances as (
   select a.account_id,a.account_number,a.account_name,a.account_group_id as group_id,coalesce(a.category,ag.category,'Sin clasificar') category,coalesce(a.nature,ag.nature,'Deudora') nature,s.name subsidiary,
   ...
   and (filter_class_id is null or exists(select 1 from journal jx join journal_line lx using(journal_id) where jx.transaction_id=g.transaction_id and lx.account_id=g.account_id and lx.class_id=filter_class_id))
   and (filter_cost_center_id is null or exists(select 1 from journal jx join journal_line lx using(journal_id) where jx.transaction_id=g.transaction_id and lx.account_id=g.account_id and lx.cost_center_id=filter_cost_center_id))
   group by a.account_id,a.account_number,a.account_name,a.account_group_id,a.category,ag.category,a.nature,ag.nature,s.name
  ), filtered as (select *,count(*) over() total_count from balances where
   (not exclude_zero or opening<>0 or debit<>0 or credit<>0 or closing<>0) and
   (p_report='trial-balance' or p_report='cash-flow' and (lower(account_name) like '%banco%' or lower(account_name) like '%caja%') or p_report='balance-sheet' and category in('Activo','Pasivo','Patrimonio') or p_report='income-statement' and category in('Ingreso','Costo','Gasto') or p_report='equity-changes' and category='Patrimonio')
  ...
  select jsonb_build_object('rows',coalesce(jsonb_agg(to_jsonb(filtered)-'total_count'),'[]'::jsonb),'total',coalesce(max(total_count),0),'page',page_no,'pageSize',page_size,
   'summary',jsonb_build_object('opening',coalesce(sum(opening),0),'debit',coalesce(sum(debit),0),'credit',coalesce(sum(credit),0),'closing',coalesce(sum(closing),0),'balanced',abs(coalesce(sum(debit),0)-coalesce(sum(credit),0))<0.005,'resultExercise',coalesce((select sum(rx.credit_amount-rx.debit_amount) from gl_impact rx join chart_accounts ax using(account_id) where rx.subsidiary_id=any(sids) and rx.posting_date<=date_to and (book_id is null or rx.accounting_book_id=book_id) and lower(ax.account_name) like '%resultado del ejercicio%'),0))) into result from filtered;
 end if;
