...
begin
 ...
 with documents as(
  ...
  select d.*,t.tran_date issue_date,t.exchange_rate,c.currency_code,sp.tax_id supplier_tax_id,sp.company_name supplier_name,sp.supplier_category,coalesce(st.code,'CONTABILIZADO') document_status,j.journal_id,
   coalesce(case when d.document_type='FAC_PRO' then (select sum(l.amount) from supplier_invoice_line l where l.invoice_id=d.document_id) when d.document_type='NC_PRO' then (select sum(l.amount) from supplier_note_line l where l.note_kind='CREDIT' and l.note_id=d.document_id) when d.document_type='ND_PRO' then (select sum(l.amount) from supplier_note_line l where l.note_kind='DEBIT' and l.note_id=d.document_id) end,d.raw_total) expense_amount,
  ...
  from documents d join "transaction" t on t.transaction_id=d.transaction_id join currencies c on c.currency_id=t.currency_id join suppliers sp on sp.supplier_id=d.supplier_id left join status st on st.status_id=t.status_id left join journal j on j.transaction_id=d.transaction_id
