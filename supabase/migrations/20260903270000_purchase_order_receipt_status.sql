...
left join purchase_document src on src.document_id=d.source_document_id
where d.subsidiary_id=active_subsidiary_id()
and d.document_type=p_filters->>'type'
and d.document_date between(p_filters->>'from')::date and(p_filters->>'to')::date
$$;
