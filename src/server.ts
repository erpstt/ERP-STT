...
import { salesWorkflowOptions, salesDocuments, salesDocumentDetail, saveSalesDocument, deleteSalesDocument } from './modules/sales-catalogs/sales-workflow.service.js';
import { customerPaymentOptions, customerPaymentReport, customerPaymentDetail, saveCustomerPayment, deleteCustomerPayment } from './modules/sales-catalogs/customer-payment.service.js';
import { saveSalesNote } from './modules/sales-catalogs/sales-note.service.js';
...
import { treasuryWorkspace, saveTreasuryOperation, saveTreasurySchedule, deleteTreasurySchedule } from './modules/treasury-catalogs/treasury-workspace.service.js';
...
import { fixedAssetOptions, fixedAssetReport, saveFixedAsset, depreciationPreview, runDepreciation } from './modules/fixed-asset-catalogs/fixed-assets.service.js';
...
const server = createServer(async (request, response) => {
  try {
    ...
    if(request.method==='GET'&&request.url==='/api/banking/deposits/options'){return json(response,200,await bankDepositOptions(request.headers.authorization!));}
    if(request.method==='GET'&&request.url==='/api/banking/checks/options'){return json(response,200,await bankCheckOptions(request.headers.authorization!));}
    if(request.method==='GET'&&request.url==='/api/banking/fees/options'){return json(response,200,await bankFeeOptions(request.headers.authorization!));}
    if(request.method==='POST'&&request.url==='/api/banking/fees'){return json(response,201,await saveBankFee(request.headers.authorization!,await body(request) as Record<string,unknown>));}
    if(request.method==='POST'&&request.url==='/api/banking/fees/import'){return json(response,201,await importBankFees(request.headers.authorization!,await body(request) as Record<string,unknown>));}
    if(request.method==='POST'&&request.url==='/api/banking/fees/report'){return json(response,200,await bankFeeReport(request.headers.authorization!,await body(request) as Record<string,unknown>));}
    const feeRoute=request.url?.match(/^\/api\/banking\/fees\/(\d+)$/);if(feeRoute){const id=Number(feeRoute[1]);if(request.method==='GET')return json(response,200,await bankFeeDetail(request.headers.authorization!,id));if(request.method==='PUT')return json(response,200,await updateBankFee(request.headers.authorization!,id,await body(request) as Record<string,unknown>));if(request.method==='DELETE')return json(response,200,await deleteBankFee(request.headers.authorization!,id));}
    if(request.method==='GET'&&request.url==='/api/banking/checks/periods'){return json(response,200,await bankCheckPeriods(request.headers.authorization!));}
    if(request.method==='POST'&&request.url==='/api/banking/checks/report'){return json(response,200,await bankCheckReport(request.headers.authorization!,await body(request) as Record<string,unknown>));}
    if(request.method==='POST'&&request.url==='/api/banking/checks'){return json(response,201,await saveBankCheck(request.headers.authorization!,await body(request) as Record<string,unknown>));}
    const checkRoute=request.url?.match(/^\/api\/banking\/checks\/(\d+)$/);if(checkRoute){const id=Number(checkRoute[1]);if(request.method==='GET')return json(response,200,await bankCheckDetail(request.headers.authorization!,id));if(request.method==='PUT')return json(response,200,await saveBankCheck(request.headers.authorization!,await body(request) as Record<string,unknown>,id));if(request.method==='DELETE')return json(response,200,await deleteBankCheck(request.headers.authorization!,id));}
    if(request.method==='POST'&&request.url==='/api/banking/deposits/report'){return json(response,200,await listBankDeposits(request.headers.authorization!,await body(request) as Record<string,unknown>));}
    ...
    const supplierPaymentRoute=request.url?.match(/^\/api\/purchasing\/supplier-payments\/(\d+)$/);if(supplierPaymentRoute){const id=Number(supplierPaymentRoute[1]);if(request.method==='GET')return json(response,200,await supplierPaymentDetail(request.headers.authorization!,id));if(request.method==='PUT')return json(response,200,await saveSupplierPayment(request.headers.authorization!,await body(request)as Record<string,unknown>,id));if(request.method==='DELETE')return json(response,200,await deleteSupplierPayment(request.headers.authorization!,id));}
    ...
    const fixedAssetManage=request.url?.match(/^\/api\/fixed-assets\/workflow-manage\/(\d+)$/);if(fixedAssetManage&&request.method==='PUT')return json(response,200,await saveFixedAsset(request.headers.authorization!,await body(request)as Record<string,unknown>,Number(fixedAssetManage[1])));
    if(request.method==='POST'&&request.url==='/api/fixed-assets/depreciation-preview'){const payload=await body(request)as Record<string,unknown>;return json(response,200,await depreciationPreview(request.headers.authorization!,String(payload.date)));}
    if(request.method==='POST'&&request.url==='/api/fixed-assets/depreciation-run')return json(response,201,await runDepreciation(request.headers.authorization!,await body(request)as Record<string,unknown>));
    if(request.method==='POST'&&request.url==='/api/fixed-assets/asset-category-report')return json(response,200,await assetCategoryReport(request.headers.authorization!,await body(request)as Record<string,unknown>));
    if(request.method==='POST'&&request.url==='/api/fixed-assets/asset-category-save')return json(response,201,await saveAssetCategory(request.headers.authorization!,await body(request)as Record<string,unknown>));
    const assetCategoryRoute=request.url?.match(/^\/api\/fixed-assets\/asset-category-manage\/(\d+)$/);if(assetCategoryRoute){const id=Number(assetCategoryRoute[1]);if(request.method==='PUT')return json(response,200,await saveAssetCategory(request.headers.authorization!,await body(request)as Record<string,unknown>,id));if(request.method==='DELETE')return json(response,200,await deleteAssetCategory(request.headers.authorization!,id));}
    if(request.method==='POST'&&request.url==='/api/treasury/workspace')return json(response,200,await treasuryWorkspace(request.headers.authorization!,await body(request)as Record<string,unknown>));
    if(request.method==='POST'&&request.url==='/api/treasury/operations')return json(response,201,await saveTreasuryOperation(request.headers.authorization!,await body(request)as Record<string,unknown>));
    if(request.method==='POST'&&request.url==='/api/treasury/schedules')return json(response,201,await saveTreasurySchedule(request.headers.authorization!,await body(request)as Record<string,unknown>));
    const treasuryScheduleRoute=request.url?.match(/^\/api\/treasury\/schedules\/(\d+)$/);if(treasuryScheduleRoute){const id=Number(treasuryScheduleRoute[1]);if(request.method==='PUT')return json(response,200,await saveTreasurySchedule(request.headers.authorization!,await body(request)as Record<string,unknown>,id));if(request.method==='DELETE')return json(response,200,await deleteTreasurySchedule(request.headers.authorization!,id));}
    ...
    const salesWorkflowRoute=request.url?.match(/^\/api\/sales\/workflow\/(\d+)$/);if(salesWorkflowRoute){const id=Number(salesWorkflowRoute[1]);if(request.method==='GET')return json(response,200,await salesDocumentDetail(request.headers.authorization!,id));if(request.method==='PUT')return json(response,200,await saveSalesDocument(request.headers.authorization!,await body(request)as Record<string,unknown>,id));if(request.method==='DELETE')return json(response,200,await deleteSalesDocument(request.headers.authorization!,id));}
    const salesNoteEntry=request.url?.match(/^\/api\/sales\/note-entry\/(CREDIT|DEBIT)(?:\/(\d+))?$/);if(salesNoteEntry&&['POST','PUT'].includes(request.method||'')){const authorization=request.headers.authorization!;return json(response,request.method==='POST'?201:200,await saveSalesNote(authorization,salesNoteEntry[1],await body(request)as Record<string,unknown>,salesNoteEntry[2]?Number(salesNoteEntry[2]):null));}
      const salesInvoiceEntryUpdate=request.url?.match(/^\/api\/sales\/invoice-entry\/(\d+)$/);if(request.method==='PUT'&&salesInvoiceEntryUpdate){const authorization=request.headers.authorization!;return json(response,200,await saveSalesInvoice(authorization,await body(request)as Record<string,unknown>,Number(salesInvoiceEntryUpdate[1])));}
      const supplierInvoiceEntryUpdate=request.url?.match(/^\/api\/purchasing\/supplier-invoice-entry\/(\d+)$/);if(request.method==='PUT'&&supplierInvoiceEntryUpdate){const authorization=request.headers.authorization!;return json(response,200,await saveSupplierInvoice(authorization,await body(request)as Record<string,unknown>,Number(supplierInvoiceEntryUpdate[1])));}
      const supplierNoteEntry=request.url?.match(/^\/api\/purchasing\/supplier-note-entry\/(CREDIT|DEBIT)(?:\/(\d+))?$/);if(supplierNoteEntry&&['POST','PUT'].includes(request.method||'')){const authorization=request.headers.authorization!;return json(response,request.method==='POST'?201:200,await saveSupplierNote(authorization,supplierNoteEntry[1],await body(request)as Record<string,unknown>,supplierNoteEntry[2]?Number(supplierNoteEntry[2]):null));}
