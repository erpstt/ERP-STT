...
export const coreCatalogs: CatalogDefinition[] = [
  ...
  { slug: 'number-sequences', table: 'number_sequences', title: 'Secuencias numéricas', description: 'Consecutivos por entidad y transacción/subsidiaria', primaryKey: 'sequence_id', fields: [{key:'scope_type',label:'Tipo de secuencia',type:'text',required:true,options:['Entidad','Transacción']},{key:'entity_key',label:'Entidad',type:'text'},{key:'transaction_type_id',label:'Tipo de transacción',type:'number',reference:'core/transaction-types'},{key:'subsidiary_id',label:'Subsidiaria',type:'number',reference:'organization/subsidiaries'},{key:'prefix',label:'Prefijo',type:'text'},{key:'suffix',label:'Sufijo',type:'text'},{key:'current_number',label:'Número actual',type:'number',required:true},{key:'padding_length',label:'Longitud',type:'number',required:true},{key:'allow_manual_update',label:'Permitir actualizar consecutivo',type:'boolean'}] },
