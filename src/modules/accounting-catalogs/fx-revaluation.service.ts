...
export async function fxRevaluation(authorization:string,action:string,payload:Record<string,unknown>={}) {
  ...
  const response=await fetchSupabase(new URL(`/rest/v1/rpc/${name}`,config.url),{method:'POST',headers:{apikey:config.anonKey,Authorization:authorization,'Content-Type':'application/json'},body:JSON.stringify(action==='options'?{}:{p_payload:payload})});
