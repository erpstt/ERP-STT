...
import {genericReportCsv,genericReportHtml} from './scheduled-report-renderer.js';
...
export type ScheduledReport={kind:string;title?:string;dateFrom?:string;data?:Record<string,unknown>;cutoff:string;company:string;currency:string;rows:Row[];summary:{subledger:number;advances:number;netBalance:number}};
...
export function scheduledReportCsv(report:ScheduledReport){
 if(!['AR','AP'].includes(report.kind))return genericReportCsv(report);
...
export async function scheduledReportsAction(authorization:string,action:string,payload:Record<string,unknown>){
 if(action==='preview'){
  ...
  const snapshot=await rpc('scheduled_report_snapshot_v2',{p_sid:data.company.id,p_kind:payload.report,p_cutoff:cutoff.toISOString().slice(0,10),p_options:{...Object(payload.reportOptions),periodMode:payload.periodMode||'YEAR_TO_DATE'}},authorization);
...
type Job={id:string;lease:string;subsidiary_id:number;cutoff:string;recipient:string;configuration:{report_kind:string;format:string;name:string;owner_id?:number;period_mode?:string;report_options?:Record<string,unknown>}};
...
export async function deliverScheduledReport(job:Job,prepare:()=>Promise<ScheduledReport>,send:(mail:any)=>Promise<any>,begin:()=>Promise<boolean>,finish:Finish,files=scheduledReportFiles){
 ...
 let result;
 try{result=await send({from:process.env.SMTP_FROM,to:job.recipient,subject:`${report.title||title(report.kind)} · ${report.company} · ${date(report.cutoff)}`.replace(/[\r\n]/g,' ').slice(0,200),text:`${report.company}\n${report.title||title(report.kind)}\nFecha de corte: ${date(report.cutoff)}${['AR','AP'].includes(report.kind)?`\nSaldo neto: ${money(report.summary.netBalance)} ${report.currency}`:''}\nAdjunto encontrará el reporte solicitado.`,html:`<div style="font-family:Arial;color:#17324d;max-width:640px"><h2>${esc(report.company)}</h2><h1>${esc(report.title||title(report.kind))}</h1><p>Adjunto encontrará el reporte programado con fecha de corte al <strong>${esc(date(report.cutoff))}</strong>.</p>${['AR','AP'].includes(report.kind)?`<p>Saldo neto: <strong>${money(report.summary.netBalance)} ${esc(report.currency)}</strong></p>`:''}<p>NEXO · ${esc(job.configuration.name)}</p></div>`,attachments:attachments.map(f=>({filename:f.fileName,content:Buffer.from(f.base64,'base64'),contentType:f.mimeType})),disableFileAccess:true,disableUrlAccess:true});}
...
export function startScheduledReports(){
 ...
 const tick=async()=>{if(busy)return;busy=true;try{
  ...
  if(job)await deliverScheduledReport(job,()=>rpc('scheduled_report_snapshot_v2',{p_sid:job.subsidiary_id,p_kind:job.configuration.report_kind,p_cutoff:job.cutoff,p_owner:job.configuration.owner_id,p_options:{...job.configuration.report_options,periodMode:job.configuration.period_mode||'YEAR_TO_DATE'}}),mail=>transport.sendMail(mail),()=>rpc('scheduled_report_begin_send',{p_id:job.id,p_lease:job.lease}),(status,message,error)=>rpc('scheduled_report_finish',{p_id:job.id,p_lease:job.lease,p_status:status,p_message:message,p_error:error}));
